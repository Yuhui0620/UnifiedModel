...
type appSettings struct {
	// APIURL is the base URL of the MModel server (cmd/mmodel-server), e.g.
	// http://192.168.31.128:8080. It must be reachable from the Grafana server
	// (container), so never "localhost" when Grafana runs in Docker.
	APIURL string `json:"apiUrl"`
}

// App is the MModel app backend. It reverse-proxies frontend resource calls to
// the configured MModel server, optionally injecting an API key (from
...
func NewApp(_ context.Context, instanceSettings backend.AppInstanceSettings) (instancemgmt.Instance, error) {
	...
	// Optional bearer token for the upstream MModel server. mmodel-server is
