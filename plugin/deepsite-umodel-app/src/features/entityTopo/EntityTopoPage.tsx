...
export function EntityTopoPage({
  ...
  const [displaySettings, setDisplaySettings] = useState<EntityTopoDisplaySettings>(
    DEFAULT_ENTITY_TOPO_DISPLAY_SETTINGS
  );
  ...
  const load = useCallback(
    async (range = queryTimeRange) => {
      ...
      try {
        ...
        const mmodelElements = (mmodelResult?.rows || [])
          .map(rowToElement)
          .filter((element) => element.kind && element.domain && element.name);
  ...
  return (
    <div className="eto-root">
      <aside className="eto-sidebar">
        ...
        {sidebarTab === 'summary' ? (
          <SummarySidebar data={data} filteredData={filteredData} filters={filters} onFiltersChange={setFilters} />
        ) : (
          <DisplaySidebar settings={displaySettings} onSettingsChange={setDisplaySettings} />
        )}
      </aside>
      ...
      <section className="eto-main">
        <header className="eto-topbar">
          ...
          <div className="eto-timebar">
            ...
            />
            <button type="button" onClick={() => applyPresetRange(15)}>
              15m
            </button>
            <button type="button" onClick={() => applyPresetRange(60)}>
              1h
            </button>
            <button type="button" onClick={() => applyPresetRange(24 * 60)}>
              24h
            </button>
        ...
        <div className="eto-content-area">
          <main className="eto-canvas-panel">
            {loading && data.nodes.length === 0 && (
              <div className="eto-loading">
                <Activity size={16} /> Loading topology...
              </div>
            )}
          ...
          <DetailPanel selection={selected} data={data} onClose={() => setSelected(null)} onFocusNode={focusNode} />
        </div>

        <footer className="eto-statusbar">
          <span>
            <strong>{filteredData.nodes.length.toLocaleString()}</strong> entities
          </span>
          <span className="eto-status-sep" />
          <span>
            <strong>{filteredData.edges.length.toLocaleString()}</strong> relations
          </span>
          {data.limitInfo.reached && (
            <>
              <span className="eto-status-sep" />
              <span className="eto-status-warning">
                <span /> limit {data.limitInfo.limit.toLocaleString()}
              </span>
            </>
          )}
        </footer>
  ...
  return (
    <div className="eto-sidebar-body eto-layout-tab">
      ...
      <div className="eto-layout-block">
        ...
        />
        <LayoutToggleRow
          label="Move entities by dragging"
          ...
          {focusPanelOpen &&
            ...
            ReactDOM.createPortal(
              <FocusDropdown
                ...
                onRemove={(id) =>
                  onFiltersChange({ ...filters, focusIds: filters.focusIds.filter((item) => item !== id) })
                }
                onClearAll={() => {
                  onFiltersChange({ ...filters, focusIds: [] });
            ...
            onRemove={() =>
              onFiltersChange({
                ...
                attributeFilters: filters.attributeFilters.filter(
                  (item) => getAttributeFilterKey(item) !== getAttributeFilterKey(filter)
                ),
              })
            }
          />
...
function FilterHelpTooltip({
  ...
  return (
    ...
    >
      ...
      <p>
        <b>Focus</b> shows selected entities and nearby entities.
      </p>
      <p>
        <b>Stack filters</b> narrows the focused view with search, type, domain, or relation filters.
      </p>
    ...
    <aside className="eto-detail-panel open">
      ...
      <div className="eto-detail-body">
        <div className="eto-detail-summary">
          <span>
            <strong>{node.inDegree}</strong> inbound
          </span>
          <span>
            <strong>{node.outDegree}</strong> outbound
          </span>
          <span>
            <strong>{node.relationCount}</strong> total
          </span>
        </div>
        <DetailTable
          rows={[
  ...
  return (
    <div className="eto-search-popover" onMouseDown={(event) => event.preventDefault()}>
      ...
      <div className="eto-search-section">
        ...
        {nodes.length === 0 ? (
          <span className="eto-search-empty">No matching entities</span>
        ) : (
          nodes.map((node) => (
...
function LayoutRadioOption({
  ...
  return (
    <button className={active ? 'eto-layout-radio active' : 'eto-layout-radio'} onClick={onClick} type="button">
      <span className="eto-layout-radio-mark">
        <i />
      </span>
...
function LayoutToggleRow({
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className="eto-layout-switch">
        <i />
      </span>
    </button>
  );
...
function FilterChip({
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      <button onClick={onRemove} type="button" aria-label={`Remove ${label}`}>
        <X size={12} />
      </button>
    </span>
  );
...
function RouteNode({
  ...
  node: EntityTopoNode;
  label: string;
  onFocus: (node: EntityTopoNode) => void;
}) {
  return (
    <button className="eto-route-node" onClick={() => onFocus(node)} type="button">
      ...
      <span>
        <b>{node.title}</b>
        <small>
          {label} · {node.endpoint.entityType}
        </small>
      </span>
    </button>
  );
...
function DetailTable({ rows }: { rows: Array<[string, unknown]> }) {
  if (rows.length === 0) {
    return <div className="eto-detail-empty">No properties.</div>;
  }
  return (
