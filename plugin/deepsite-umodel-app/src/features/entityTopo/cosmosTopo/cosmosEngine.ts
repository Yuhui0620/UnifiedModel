...
function getTimelineState(presence: number[] | undefined, playhead: number): 'active' | 'past' | 'future' | undefined {
  if (!presence || presence.length === 0) {
    return undefined;
  }
  const currentIndex = Math.floor(playhead);
  if (timelinePresenceHas(presence, currentIndex) || timelinePresenceHas(presence, currentIndex + 1)) {
    return 'active';
  }
...
function getTimelineOpacity(
  ...
  if (!presence || !Number.isFinite(Number(playhead))) {
    return undefined;
  }
...
function buildHexOffsets(count: number, spacing: number): Array<[number, number]> {
  if (count <= 0) {
    return [];
  }
  ...
  for (let ring = 1; offsets.length < count; ring += 1) {
    ...
    for (const [dq, dr] of directions) {
      for (let step = 0; step < ring && offsets.length < count; step += 1) {
        offsets.push([spacing * (q + r / 2), spacing * (Math.sqrt(3) / 2) * r]);
        q += dq;
        r += dr;
      }
    }
...
function relaxCircleCenters(centers: Map<string, [number, number]>, radii: Map<string, number>, padding: number): void {
...
export class CosmosEngine {
  ...
  loadData(data: TopoData, layout: LayoutOptions): void {
    ...
    if (this.isRelationsMode(layout)) {
      this.computeRelationPositions(data);
    }
    this.destroyGraph();
  ...
  updateData(data: TopoData): void {
    ...
    this.buildIndex(data);
    if (preserveView) {
      this.restoreStableNodePositions(previousPositions);
    }
    if (!preserveView && this.isRelationsMode(this.currentLayout)) {
      this.computeRelationPositions(data);
    }
    if (!preserveView) {
      this.zoomBaseline = 0;
    ...
    if (this.isForceMode(this.currentLayout)) {
      if (preserveView) {
        ...
        this.renderStaticForceLayout(300, !skipFitView);
      }
    } else {
      this.graph.render(0);
    }
  ...
  setRuntimeLayout(layout: LayoutOptions): void {
    ...
    if (Number.isFinite(Number(nextPlayhead)) && nextPlayhead !== previousPlayhead && this.hasTimelinePresence()) {
      this.applyTimelinePlayhead(
        Number(nextPlayhead),
        !nextTimelinePlaying || previousTimelinePlaying !== nextTimelinePlaying
      );
      return;
    }
    if (
      previousTimelinePlaying !== nextTimelinePlaying &&
      Number.isFinite(Number(nextPlayhead)) &&
      this.hasTimelinePresence()
    ) {
  ...
  updateLayout(layout: LayoutOptions): void {
    ...
    if (this.isRelationsMode(layout)) {
      ...
      if (this.shouldClusterByType(layout)) {
        this.computeClusteredPositions({ nodes: this.rawNodes, edges: this.edges });
      } else {
        this.computeForcePositions({ nodes: this.rawNodes, edges: this.edges });
      }
  ...
  filterByClusters(keys: string[] | null): void {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      if (!activeSet.has(this.nodes[i].cluster)) {
        pc[i * 4 + 3] = 0;
      }
    }
  ...
  fitViewByNodeIds(ids: string[], duration = 300): void {
    ...
    this.zoomBaseline = 0;
    if (indices.length) {
      this.graph.fitViewByPointIndices(indices, duration, 0.12);
    } else {
  ...
  zoomToNode(nodeId: string, duration = 500): void {
    ...
    const idx = this.nodeIdToIndex.get(nodeId);
    if (idx !== undefined) {
      this.graph.zoomToPointByIndex(idx, duration, 3);
    }
  }
  ...
  getViewState(): TopoGraphViewState | null {
    ...
    try {
      zoomLevel = this.graph.getZoomLevel();
    } catch {
      /* Keep the last tracked zoom. */
    }
    ...
    return {
      ...
      transform: hasTransform ? { x: Number(transform.x), y: Number(transform.y), k: Number(transform.k) } : undefined,
    };
  }
  ...
  restoreViewState(state: TopoGraphViewState | null | undefined, duration = 0): void {
    ...
    this.zoomBaseline =
      Number.isFinite(nextBaseline) && nextBaseline > 0 ? clamp(nextBaseline, minZoom, maxZoom) : this.zoomLevel;
    this.lastVisualScaleKey = '';
    this.applyAdaptiveNodeSizes(true);
  ...
  getNodes(): readonly NodeRecord[] {
    return this.nodes;
  }
  getRawNode(id: string): TopoNode | undefined {
    return this.rawNodeById.get(id);
  }
  getClusters(): readonly ClusterRecord[] {
    return this.clusters;
  }
  getHoverState(): HoverState {
    return this.hover;
  }
  getNodeIdToIndex(): ReadonlyMap<string, number> {
    return this.nodeIdToIndex;
  }
  getNodeById(): ReadonlyMap<string, NodeRecord> {
    return this.nodeById;
  }
  getNeighbors(): ReadonlyMap<string, Set<string>> {
    return this.neighbors;
  }
  getClusterPositionsMap(): ReadonlyMap<string, [number, number]> {
    return this.clusterPositions;
  }
  getLayout(): LayoutOptions {
    return this.currentLayout;
  }
  getEdges(): readonly TopoEdge[] {
    return this.edges;
  }
  getValidEdgeIndices(): readonly number[] {
    return this.validEdgeIndices;
  }

  getPointPositionsForRender(): ArrayLike<number> | null {
    if (this.canUseCachedHitPositions()) {
      if (this.pointPositions.length === this.nodes.length * 2) {
    ...
    try {
      return this.graph.getPointPositions();
    } catch {
      return null;
    }
  }
  ...
  getVectorIconOverlays(maxItems = 5000): CosmosVectorIconOverlayItem[] {
    ...
    const hasFocus = Boolean(focusNodeId) || Boolean(linkSourceId && linkTargetId);
    ...
    for (const node of this.nodes) {
      ...
      if (node.index * 2 + 1 >= positions.length) {
        continue;
      }
      const baseOpacity = clamp(node.opacity ?? 1, 0, 1);
      if (baseOpacity <= 0.025) {
        continue;
      }
      ...
      if (!Number.isFinite(screen[0]) || !Number.isFinite(screen[1])) {
        continue;
      }
      if (screen[0] < -margin || screen[0] > width + margin || screen[1] < -margin || screen[1] > height + margin) {
        continue;
      }
  ...
  getLockedNodeAction(): CosmosNodeActionInfo | null {
    ...
    if (!node || !node.vectorIconVisible) {
      return null;
    }
  ...
  getFocusedEdges(limit = 12): FocusedEdgeRecord[] {
    ...
    if (!focusedNodeId || this.hover.hoverProgress <= 0) {
      return [];
    }
    ...
    for (let linkIndex = 0; linkIndex < this.validEdgeIndices.length; linkIndex += 1) {
      ...
      if (!edge || (edge.source !== focusedNodeId && edge.target !== focusedNodeId)) {
        continue;
      }
  ...
  destroy(): void {
    ...
    this.clearZoomBaselineTimer();
    if (this.labelSyncRafId !== null) {
      cancelAnimationFrame(this.labelSyncRafId);
    }
    if (this.hoverTrackRafId !== null) {
      cancelAnimationFrame(this.hoverTrackRafId);
    }
    ...
    if (this.visualScaleRafId !== null) {
      cancelAnimationFrame(this.visualScaleRafId);
    }
    this.destroyGraph();
  }
  ...
  private shouldClusterByType(layout: LayoutOptions): boolean {
    if (typeof layout.clusterByType === 'boolean') {
      return layout.clusterByType;
    }
    return layout.mode === 'clustered' || layout.mode === 'graphviz';
  }

  private shouldFocusOnHover(): boolean {
    if (this.currentLayout.hoverMode === 'focus') {
      return true;
    }
    if (this.currentLayout.hoverMode === 'panel') {
      return false;
    }
  ...
  private zoomToNearestPoint(position: [number, number], duration: number, zoom: number): void {
    ...
    if (!this.canUseCachedHitPositions()) {
      ...
      try {
        positions = this.graph.getPointPositions();
      } catch {
        return;
      }
  ...
  private getAdaptiveZoomBaseline(zoom: number): number {
    ...
    if (this.currentLayout.forceDetailedView === true) {
      return zoom;
    }
    ...
    this.graph = new Graph(this.container, {
      ...
      onPointMouseOver: (index: number, _pos: [number, number] | undefined, ev: any) => {
        ...
        if (
          ...
          !this.isNodeIndexAtScreenPosition(index, ev, this.getNodeHitRadius(index, 4, 18))
        ) {
          return;
        }
      ...
      onPointMouseOut: () => {
        ...
        if (this.lastPointerPosition && this.findNodeAtScreenPoint(this.lastPointerPosition, 4, 18) !== undefined) {
          return;
        }
      ...
      onLinkMouseOver: (linkIndex: number) => {
        ...
        this.cancelHoverTimer();
        this.cancelHoverFade();
        if (this.hover.hoveredLinkIndex === linkIndex && this.hover.hoverProgress > 0) {
          return;
        }
        this.hover.hoveredLinkIndex = linkIndex;
        this.hover.hoveredNodeId = undefined;
  ...
  private handleCanvasClick(nativePointIndex: number | undefined, event: MouseEvent): void {
    ...
    if (this.hover.lockedNodeId || this.hover.lockedLinkIndex !== undefined) {
      ...
      const clickedLockedNativeNode =
        nativePointIndex !== undefined && this.indexToNodeId.get(nativePointIndex) === this.hover.lockedNodeId;
  ...
  private handlePointerDown = (event: PointerEvent): void => {
    ...
    if (this.currentLayout.enableDrag === false) {
      return;
    }
  ...
  private handlePointerMove = (event: PointerEvent): void => {
    ...
    if (!state || event.pointerId !== state.pointerId || !this.graph) {
      return;
    }
  ...
  private flushPointerTrack = (): void => {
    ...
    if (!point || this.hover.lockedNodeId || this.hover.lockedLinkIndex !== undefined) {
      return;
    }
    if (this.hover.hoveredLinkIndex !== undefined) {
      return;
  ...
  private moveNodeToScreenPosition(index: number, screenPosition: [number, number]): void {
    ...
    try {
      next = this.graph.screenToSpacePosition(screenPosition);
    } catch {
      return;
    }
  ...
  private dispatchNodeClick(index: number): void {
    ...
    if (rawNode) {
      this.callbacks.onNodeClick?.(rawNode);
    }
  }
  ...
  private screenPointToSpace(point: [number, number]): [number, number] | undefined {
    ...
    try {
      return this.graph.screenToSpacePosition(point);
    } catch {
      return undefined;
    }
  }
  ...
  private getViewNodePosition(index: number): [number, number] | undefined {
    if (this.canUseCachedHitPositions()) {
      return this.getNodeSpacePosition(index);
    }
    if (!this.graph) {
      return undefined;
    }
  ...
  private findNodeAtScreenPoint(point: [number, number], minRadius = 5, maxRadius = 20): number | undefined {
    if (!this.canUseCachedHitPositions()) {
      return this.findNodeAtScreenPointFromGraph(point, minRadius, maxRadius);
    }
  ...
  private findNodeAtScreenPointFromGraph(point: [number, number], minRadius = 5, maxRadius = 20): number | undefined {
    ...
    for (const node of this.nodes) {
      ...
      let screen: [number, number];
      try {
        screen = this.graph.spaceToScreenPosition([positions[i * 2], positions[i * 2 + 1]]);
      } catch {
        continue;
      }
  ...
  private isNodeIndexAtScreenPositionFromGraph(index: number, event: MouseEvent, radius: number): boolean {
    ...
    if (index * 2 + 1 >= positions.length) {
      return false;
    }
    let screen: [number, number];
    try {
      screen = this.graph.spaceToScreenPosition([positions[index * 2], positions[index * 2 + 1]]);
    } catch {
      return false;
    }
  ...
  private findLinkAtScreenPosition(event: MouseEvent): number | undefined {
    if (!this.canUseCachedHitPositions()) {
      return this.findLinkAtScreenPositionFromGraph(event);
    }
    const point = this.screenPointToSpace(this.getEventOffset(event));
    if (!point) {
      return undefined;
  ...
  private findLinkAtScreenPositionFromGraph(event: MouseEvent): number | undefined {
    ...
    for (let linkIndex = 0; linkIndex < this.validEdgeIndices.length; linkIndex += 1) {
      ...
      if (sourceIndex * 2 + 1 >= positions.length || targetIndex * 2 + 1 >= positions.length) {
        continue;
      }
  ...
  private isLinkIndexAtScreenPosition(linkIndex: number, event: MouseEvent, maxDistance: number): boolean {
    if (!this.canUseCachedHitPositions()) {
      return this.isLinkIndexAtScreenPositionFromGraph(linkIndex, event, maxDistance);
    }
    const edgeIndex = this.validEdgeIndices[linkIndex];
    const edge = this.edges[edgeIndex];
    if (!edge) {
  ...
  private captureNodePositions(): Map<string, [number, number]> {
    ...
    this.nodes.forEach((node) => {
      if (Number.isFinite(node.x) && Number.isFinite(node.y)) {
        positions.set(node.id, [node.x, node.y]);
      }
    });
    return positions;
  }
  ...
  private restoreStableNodePositions(previousPositions: Map<string, [number, number]>): void {
    if (previousPositions.size === 0 || this.nodes.length === 0) {
      return;
    }
    ...
    this.nodes.forEach((node) => {
      ...
      const neighborPositions: Array<[number, number]> = [];
      this.edges.forEach((edge) => {
        ...
        if (neighbor && restored.has(neighbor.id)) {
          neighborPositions.push([neighbor.x, neighbor.y]);
        }
      });
  ...
  private buildIndex(data: TopoData): void {
    ...
    const resolveHashColor = createTopoHashColorResolver(
      data.nodes.map((n) =>
    ...
    this.nodes = data.nodes.map((n, i) => {
      ...
      if (n.data?.subTitle) {
        cr.label = n.data.subTitle;
      }
      const baseOpacity = readOpacity(rawStyle.opacity, rawStyle.timelineOpacity, (n as any).opacity);
      const baseSizeScale = Number.isFinite(Number(rawStyle.sizeScale))
        ? clamp(Number(rawStyle.sizeScale), 0.18, 1.6)
        : 1;
    ...
    this.nodes.forEach((n) => {
      n.degree = this.neighbors.get(n.id)?.size ?? 0;
    });
    ...
    if (this.isForceMode(this.currentLayout)) {
      if (this.shouldClusterByType(this.currentLayout)) {
        this.computeClusteredPositions(data);
      } else {
        this.computeForcePositions(data);
      }
    } else {
      this.computeClusteredPositions(data);
    }
  ...
  private computeForcePositions(data: TopoData): void {
    ...
    components.forEach((component, componentIndex) => {
      ...
      positions.forEach(([x, y]) => {
        radius = Math.max(radius, Math.hypot(x, y) + spacing * 2.2);
      });
  ...
  private computeForceComponentOffsets(
    ...
    for (let head = 0; head < queue.length; head += 1) {
      ...
      adjacency.get(id)?.forEach((nextId) => {
        if (!componentIds.has(nextId) || visited.has(nextId)) {
          return;
        }
        visited.add(nextId);
    ...
    Array.from(byLevel.entries())
      ...
      .forEach(([level, nodes]) => {
        ...
        sorted.forEach((node, index) => {
          ...
          positions.set(node.id, [Math.cos(angle) * (ringRadius + jitter), Math.sin(angle) * (ringRadius + jitter)]);
        });
      });
  ...
  private applyAsyncGraphvizLayout(data: TopoData, layout: LayoutOptions): void {
    ...
    computeGraphvizLayout(data, {
      ...
      .then((result) => {
        if (this.destroyed || runId !== this.graphvizLayoutRunId || !this.graph) {
          return;
        }
        let changed = false;
        this.nodes.forEach((node) => {
        ...
        this.buildPointPositions();
        this.graph.render(0);
        if (!layout.skipFitViewOnDataUpdate) {
          this.fitView(360);
        }
        this.scheduleLabelSync();
      })
  ...
  private getTimelineEdgeOpacity(
    edge: TopoEdge,
    edgeIndex: number,
    playhead: number | undefined = this.currentLayout.timelinePlayhead
  ): number | undefined {
  ...
  private applyTimelinePlayhead(playhead: number, syncLabels = true): void {
    ...
    this.nodes.forEach((node) => {
      ...
      node.disableVectorIcon =
        node.baseDisableVectorIcon || (!forceDetailedIcons && (timelinePlaying || timelineOpacity < 0.62));
  ...
  private writePointBaseColors(): void {
    ...
    const buf = this.pointBaseColors.length === length ? this.pointBaseColors : new Float32Array(length);
    if (this.pointBaseRgb.length !== this.nodes.length * 3) {
      this.buildPointBaseRgb();
    }
    this.nodes.forEach((n, i) => {
      const o = i * 4;
  ...
  private getAdaptiveNodeProfile(): {
    ...
    if (layoutKind === 'graphviz') {
      ...
      zoomOffset = 0.56;
      zoomSpan = 1.9;
      iconFactor = count >= 2500 ? 0.46 : 0.3;
    ...
    const iconThreshold = Math.max(IMAGE_VISIBILITY_MIN_SIZE, iconFloor, minSize + (maxSize - minSize) * iconFactor);
  ...
  private applyAdaptiveNodeSizes(force = false, shouldRender = true): void {
    ...
    if (!force && key === this.lastVisualScaleKey) {
      return;
    }

    const colorSource = this.pointBaseColors.length ? this.pointBaseColors : this.pointColors;
    const pointSizes =
      this.pointSizes.length === this.nodes.length ? this.pointSizes : new Float32Array(this.nodes.length);
    const pointColors =
      this.pointColors.length === colorSource.length ? this.pointColors : new Float32Array(colorSource.length);
    ...
    this.nodes.forEach((node, index) => {
      ...
      if (hasIcons) {
        ...
        imageSizes[index] = showIcon ? Math.max(0, roundToHalf(screenSize - profile.iconInset)) : screenSize;
      }
    });
    ...
    if (hasIcons) {
      this.pointImageSizes = imageSizes;
    }
  ...
  private buildLinks(): void {
    ...
    this.edges.forEach((e, eIdx) => {
      ...
      colors.push(br, bg, bb, linkOpacity * edgeOpacity);
    });
  ...
  private buildLinkColorsOnly(): void {
    if (!this.graph || this.validEdgeIndices.length === 0) {
      return;
    }
    const linkOpacity = this.getBaseLinkOpacity();
    const lc =
      this.linkColors.length === this.validEdgeIndices.length * 4
        ? this.linkColors
  ...
  private cancelHoverFade(): void {
    if (this.hoverFadeRaf !== null) {
      cancelAnimationFrame(this.hoverFadeRaf);
  ...
  private scheduleHover(nodeId: string): void {
    if (this.hover.hoveredNodeId === nodeId && this.hover.hoverProgress > 0) {
      return;
    }
    this.cancelHoverTimer();
  ...
  private applyHoverColors(t: number): void {
    if (!this.graph || !this.hover.hoveredNodeId) {
      return;
    }
