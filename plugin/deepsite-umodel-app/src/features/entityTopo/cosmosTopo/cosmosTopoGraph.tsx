...
function withFocusedDetailLayout(
  base: LayoutOptions,
  renderData: TopoData,
  ...
  const forceDetailedView = Boolean(
    ...
    (isolatedNodeId && renderData.nodes.length > 0 && renderData.nodes.length <= FOCUSED_DETAIL_NODE_LIMIT)
  );
  return forceDetailedView ? { ...base, forceDetailedView, showLabels: true } : { ...base, forceDetailedView: false };
}
    ...
    >
      {iconUrl ? (
        <img
          ...
          alt=""
          draggable={false}
          style={{ width: innerSize, height: innerSize, display: 'block', objectFit: 'contain' }}
        />
      ) : hasSvgSymbol ? (
...
function CosmosVectorIconOverlay({ engine, tick }: { engine: CosmosEngine | null; tick: number }) {
  // `tick` re-subscribes this memo to the imperative engine's per-frame state (intentional trigger).
    ...
    >
      <div
        style={{
          ...
          alignItems: 'center',
          gap: 10,
          marginBottom: info.kind === 'edge' ? 10 : 9,
          minWidth: 0,
        }}
      >
        {info.kind === 'node' ? (
...
function CosmosMiniMap({ engine, tick }: { engine: CosmosEngine | null; tick: number }) {
  // Throttled to every 5th engine tick; reads fresh imperative engine state (intentional trigger).
  // eslint-disable-next-line react-hooks/exhaustive-deps
...
export const CosmosTopoGraph = forwardRef<TopoGraphRef, Omit<TopoGraphProps, 'type'>>((props, ref) => {
  ...
  const graphApiRef = useRef<TopoGraphApi | null>(null);
  const mountedRef = useRef(false);
  const theme = useTheme2();
  const bgColorRef = useRef(theme.colors.background.primary);
  bgColorRef.current = theme.colors.background.primary;
  ...
  useEffect(() => {
    if (props.layout) {
      ...
      setLayout(next);
      engineRef.current?.setRuntimeLayout(
        withFocusedDetailLayout(next, renderDataRef.current, isolatedNodeIdRef.current)
      );
      finishRendering(seq);
    }
  ...
  useEffect(() => {
    ...
    // Engine is intentionally recreated only on layout/data identity changes, not when
    // stable callbacks (finishRendering/requestTick) change.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    layout.mode,
  ...
  const isolateHoveredNode = useCallback(
    (nodeId: string) => {
      const seq = beginRendering();
  ...
  const getGraphApi = useCallback((): TopoGraphApi => {
    ...
    const api: TopoGraphApi = {
      ...
      setIsolationLayout: () => {
        /* cosmos uses GPU layout, no CPU isolation layout switch */
      },
      ...
      render: async () => {
        engineRef.current?.getGraph()?.render();
      },
      fitView: async () => {
        engineRef.current?.fitView();
      },
  ...
  return (
    <div style={{ ...rootStyle, ...props.style }} className={props.className}>
      ...
      <CosmosLabels
        ...
        isIsolationMode={
          Boolean(isolatedNodeId) ||
          Boolean(layout.forceDetailedView && renderData.nodes.length <= FOCUSED_DETAIL_NODE_LIMIT)
        }
        showLabels={
          Boolean(
            (isolatedNodeId || layout.forceDetailedView) && renderData.nodes.length <= FOCUSED_DETAIL_NODE_LIMIT
