...
export function createTopoHashColorResolver(seeds: string[]) {
  ...
  return (seed: string) =>
    assigned.get(seed) || PALETTE[Math.floor(hashToUnit(seed) * PALETTE.length) % PALETTE.length];
}
...
export function resolveTopoVisualIdentity({
  ...
  const iconFill =
    readString(style?.iconFill) || readString(style?.fill) || readString(data?.color) || fallbackColor || '#6366f1';
  const iconPreset =
...
export function resolveLucideIconPreset(value?: string): TopoIconPreset | undefined {
  ...
  if (normalized.includes('server') || normalized.includes('box')) {
    return 'instance';
  }
  if (normalized.includes('network') || normalized.includes('globe')) {
    return 'network';
  }
  if (normalized.includes('route') || normalized.includes('workflow')) {
    return 'service';
  }
  if (normalized.includes('arrow') || normalized.includes('endpoint')) {
    return 'endpoint';
  }
  return undefined;
}
...
function inferPreset(value: string): TopoIconPreset {
  ...
  if (text.includes('operation') || text.includes('endpoint') || text.includes('http')) {
    return 'endpoint';
  }
  ...
  if (text.includes('instance') || text.includes('host') || text.includes('node')) {
    return 'instance';
  }
  if (text.includes('pod')) {
    return 'pod';
  }
  if (text.includes('network') || text.includes('vpc')) {
    return 'network';
  }
  return 'default';
}
