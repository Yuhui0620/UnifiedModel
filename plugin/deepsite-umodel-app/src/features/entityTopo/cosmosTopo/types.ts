...
export interface CosmosEngineConfig {
  ...
  /** WebGL canvas background color (derived from the Grafana theme). */
...
export interface TopoGraphApi {
  ...
  on: (eventName: string, callback: (...args: any[]) => void) => TopoGraphApi;
  once: (eventName: string, callback: (...args: any[]) => void) => TopoGraphApi;
  off: (eventName: string, callback: (...args: any[]) => void) => TopoGraphApi;
}

export interface TopoGraphRef {
  getGraph: () => TopoGraphApi;
}
