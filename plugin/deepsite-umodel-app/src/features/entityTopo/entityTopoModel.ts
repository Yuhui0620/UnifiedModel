...
export type TopoSelection = { kind: 'node'; node: EntityTopoNode } | { kind: 'edge'; edge: EntityTopoEdge };
...
export function buildEntityTopoData(
  ...
  const nodes = nodeValues
    ...
    .sort(
      (left, right) =>
        left.endpoint.cluster.localeCompare(right.endpoint.cluster) || left.title.localeCompare(right.title)
    );
...
export function filterEntityTopoData(data: EntityTopoData, filters: EntityTopoFilters): EntityTopoData {
  ...
  if (!shouldApplySecondaryFilters) {
    return focusData;
  }
  ...
  const relationMatches = (edge: EntityTopoEdge) =>
    filters.relations.length === 0 || filters.relations.includes(edge.relationType);
  const edges = focusData.edges.filter(
    (edge) => (nodeMatches.has(edge.source) || nodeMatches.has(edge.target)) && relationMatches(edge)
  );
...
function titleFromProperties(properties: Record<string, unknown>, endpoint: EntityEndpoint) {
  ...
  const fallback = Object.entries(properties).find(([key, value]) => {
    if (/^(id|entity_id|uid|uuid)$/i.test(key)) {
      return false;
    }
    if (/_id$/i.test(key)) {
      return false;
...
function buildClusterMetas(nodes: EntityTopoNode[], mmodelIndex: Map<string, { displayName: string }>) {
  ...
  for (const node of nodes) {
    counts.set(node.endpoint.cluster, (counts.get(node.endpoint.cluster) || 0) + 1);
  }
  return [...counts.keys()]
...
function createMModelEntityIndex(elements: MModelElement[]) {
  ...
  for (const element of elements) {
    if (element.kind !== 'entity_set') {
      continue;
    }
    const displayName = displayNameForMModel(element);
    const domain = element.domain || 'unknown';
    const name = element.name || '';
    const keys = [mmodelElementKey(element), `${domain}@${name}`, `${domain}/${name}`, name].filter(Boolean);
    for (const key of keys) {
      index.set(key, { displayName });
    }
  }
  return index;
}
...
function recomputeDomains(nodes: EntityTopoNode[]) {
  const counts = new Map<string, number>();
  for (const node of nodes) {
    counts.set(node.endpoint.domain, (counts.get(node.endpoint.domain) || 0) + 1);
  }
  return [...counts.entries()]
...
function buildAttributeAggregations(nodes: EntityTopoNode[]): AttributeAggregationIndex {
  ...
  for (const node of nodes) {
    ...
    Object.entries(node.properties).forEach(([field, rawValue]) => {
      if (!isAttributeField(field, rawValue)) {
        return;
      }
  ...
  for (const [cluster, aggregation] of clusterMaps.entries()) {
    const fields = [...aggregation.fields.entries()]
      ...
      .filter((field) => {
        ...
        if (field.distinctCount > 80 && field.distinctCount / Math.max(1, aggregation.totalCount) > 0.65) {
          return false;
        }
        return true;
      })
      .sort(
        (left, right) =>
          right.count - left.count || left.distinctCount - right.distinctCount || left.field.localeCompare(right.field)
      );
    result[cluster] = {
      cluster,
...
function isAttributeField(field: string, value: unknown) {
  if (/^(__|id$|entity_id$|uid$|uuid$)/i.test(field)) {
    return false;
  }
  if (/_id$/i.test(field) && field !== 'trace_id') {
    return false;
  }
  if (value === null || value === undefined) {
    return false;
...
function nodePassesScope(node: EntityTopoNode, filters: EntityTopoFilters) {
  ...
  if (filters.domains.length > 0 && !filters.domains.includes(node.endpoint.domain)) {
    return false;
  }
  return true;
}
...
function nodeMatchesAttributeFilter(node: EntityTopoNode, filter: AttributeTopoFilter) {
  if (node.endpoint.cluster !== filter.cluster) {
    return false;
  }
...
function compactSearchText(values: unknown[]) {
  ...
  const visit = (value: unknown, depth: number) => {
    if (parts.length >= 80 || value == null || depth > 2) {
      return;
    }
