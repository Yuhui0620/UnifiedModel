...
export function GraphView({
  ...
  onNodesChange: (changes: Array<NodeChange<Node<ExplorerNodeData>>>) => void;
  onSelect: (element: MModelElement | null) => void;
}) {
  const { fitView } = useReactFlow();
  const theme = useTheme2();
  const gridColor = theme.colors.border.medium;
  const gridColorWeak = theme.colors.border.weak;
  ...
  const displayEdges = useMemo(
    () =>
      graph.edges.map((edge) => ({
        ...edge,
        selected: selectedId === (edge.data?.element ? elementKey(edge.data.element) : ''),
      })),
    [graph.edges, selectedId]
  );

  useEffect(() => {
    if (graph.nodes.length === 0 || layouting) {
      return;
    }
    const focusNodes = focusIds.length > 0 ? graph.nodes.filter((node) => focusIds.includes(node.id)) : [];
    const timer = window.setTimeout(() => {
      if (focusNodes.length > 0) {
        fitView({ duration: 800, padding: 0.2, nodes: focusNodes });
      } else {
        fitView({ duration: 800, padding: 0.2 });
      }
    }, 120);
    return () => window.clearTimeout(timer);
    // focusKey/nodeIdsKey are derived string keys that intentionally gate re-fitting; the raw
    // focusIds/graph.nodes arrays are read through them.
    ...
    <div className="v2-graph-container" data-zoom={forceFullMode ? 'full' : zoomLevel}>
      ...
      >
        {backgroundStyle === 'dots' && (
          <Background variant={BackgroundVariant.Dots} gap={28} size={2} color={gridColor} />
        )}
        {backgroundStyle === 'lines' && (
          <Background variant={BackgroundVariant.Lines} gap={120} size={1.5} color={gridColorWeak} />
        )}
        {backgroundStyle === 'cross' && (
          <Background variant={BackgroundVariant.Cross} gap={28} size={8} color={gridColor} />
        )}
        ...
        <MiniMap
          ...
          pannable
          zoomable
          nodeColor={theme.colors.text.secondary}
          style={{ background: 'var(--ume-color-bg-subtle)' }}
        />
      </ReactFlow>
    ...
    menuOpen && menuRect
      ? ReactDOM.createPortal(
          ...
          >
            ...
            <button
              className="danger"
              onClick={() => runMenuAction(() => data.actions.onDelete(data.element, true))}
              type="button"
            >
              <Trash2 size={13} />
              Delete node
  ...
  return (
    <>
      ...
      {data && label && (
        <EdgeLabelRenderer>
          <div
            className={`ume-edge-label ${data.draftStatus ? `draft-${data.draftStatus}` : ''}`}
