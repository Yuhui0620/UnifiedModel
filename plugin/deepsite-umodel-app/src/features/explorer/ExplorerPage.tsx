...
import { buildGraph, layoutGraphWithGraphviz, type ExplorerNodeData, type GraphModel } from './graphModel';
...
export function ExplorerPage({
  ...
  const copyDraftElement = useCallback(
    (element: MModelElement) => {
  ...
  const copyDraftCascadeElement = useCallback(
    (element: MModelElement) => {
      const copy = cloneElementForDraft(element);
      updateDraft(
        (items) => {
        ...
        isLinkElement(element)
          ? `Added draft copy: ${titleForElement(copy)}`
          : `Added draft copy with related links: ${titleForElement(copy)}`
      );
      setSelected(copy);
      setMode('graph');
      setFocusIds([elementKey(element), elementKey(copy)]);
  ...
  const deleteDraftElement = useCallback(
    (element: MModelElement, cascade: boolean) => {
      updateDraft(
        (items) => {
          const ids = new Set<string>([elementKey(element)]);
          if (cascade && !isLinkElement(element)) {
            for (const item of items) {
              if (isLinkElement(item) && linkTouchesElement(item, element, items)) {
                ids.add(elementKey(item));
              }
            }
          }
          return items.filter((item) => !ids.has(elementKey(item)));
        },
        cascade ? 'Deleted draft node and related links' : 'Deleted draft element'
      );
  ...
  useEffect(() => {
    function handleKeyboardDelete(event: KeyboardEvent) {
      if (!selected || diffOpen || createOpen || uploadOpen || connectOpen) {
        return;
      }
      ...
      if (target?.closest('input, textarea, select, [contenteditable="true"], .monaco-editor')) {
        return;
      }
  ...
  const replaceDraftElement = useCallback(
    (next: MModelElement) => {
  ...
  const createDraftElement = useCallback(
    (next: MModelElement) => {
  ...
  const uploadDraftElements = useCallback(
    (items: MModelElement[]) => {
      updateDraft(
        (current) => {
          let next = current;
        ...
        `Uploaded ${items.length} draft element${items.length === 1 ? '' : 's'}`
      );
  ...
  const createDraftLinks = useCallback(
    (source: MModelElement, targets: MModelElement[]) => {
      if (targets.length === 0) {
        return;
      }
      ...
      updateDraft(
        (items) => {
          ...
          for (const link of links) {
            next = upsertById(next, link);
          }
          return next;
        },
        `Created ${links.length} draft link${links.length === 1 ? '' : 's'} from ${titleForElement(source)}`
      );
      setSelected(links[0]);
      setMode('graph');
  ...
  const draftStatusById = useMemo(() => {
    ...
    for (const element of diff.added) {
      status.set(elementKey(element), 'added');
    }
    for (const element of diff.modified) {
      status.set(elementKey(element), 'modified');
    }
    return status;
  }, [diff]);
  ...
  useEffect(() => {
    ...
    if (mode !== 'graph' || graphSource.nodes.length === 0) {
      return;
    }
    setLayouting(true);
  ...
  const handleGraphNodesChange = useCallback((changes: Array<NodeChange<Node<ExplorerNodeData>>>) => {
    setGraph((current) => ({
      ...current,
  ...
  const activeCount = kindFilters.length + domainFilters.length + focusIds.length + fullTextFilters.length;
  ...
  async function importSample() {
    ...
    try {
      ...
      await load();
      setMessage(
        `Imported sample: ${result.mmodel.imported} MModel elements, ${result.entities.accepted} entities, ${result.relations.accepted} relations.`
      );
    } catch (nextError) {
      setError(formatError(nextError));
    } finally {
  ...
  async function commitDraft() {
    ...
    try {
      ...
      if (upserts.length > 0) {
        ...
        if (!validation.valid) {
          setError(
            (validation.errors || []).map((item) => `${item.field || 'element'}: ${item.reason}`).join('\n') ||
              'Validation failed'
          );
          return;
        }
      ...
      setDiffOpen(false);
      await load();
      setMessage(`Submitted ${upserts.length} upserts and ${diff.deleted.length} deletes.`);
    } catch (nextError) {
      setError(formatError(nextError));
    } finally {
      setCommitting(false);
    ...
    <div className="ume-v2 openmmodel-explorer">
      ...
      <section className="ume-main">
        ...
        <footer className="ume-statusbar">
          <span>
            <strong>{filteredStats.nodes}</strong> nodes
          </span>
          <span className="ume-status-sep" />
          <span>
            <strong>{filteredStats.links}</strong> links
          </span>
          {resultLimitReached && resultLimit && (
            <>
              <span className="ume-status-sep" />
              <span>limit {resultLimit.toLocaleString()}</span>
            </>
          )}
        </footer>
      </section>
...
function TableView({
  ...
  function sortArrow(key: SortKey) {
    if (sortKey !== key) {
      return <span className="ume-sort-arrow muted">↕</span>;
    }
        ...
        <table className="ume-data-table">
          <thead>
            <tr>
              ...
              <th className="col-name" onClick={() => changeSort('name')} role="button">
                Name {sortArrow('name')}
              </th>
              <th className="col-domain" onClick={() => changeSort('domain')} role="button">
                Domain {sortArrow('domain')}
              </th>
              <th className="col-type" onClick={() => changeSort('kind')} role="button">
                Type {sortArrow('kind')}
              </th>
              <th className="col-description" onClick={() => changeSort('description')} role="button">
          ...
          <tbody>
            {pagedRows.map((element) => {
              ...
              return (
                <tr
                  key={key}
                  className={selected && elementKey(selected) === key ? 'selected' : ''}
                  onClick={() => onSelect(element)}
                >
                  <td className="cell-icon">
                    <span
                      className={isLinkElement(element) ? 'ume-kind-line table' : 'ume-kind-dot table'}
            ...
            {pagedRows.length === 0 && (
              <tr>
                <td colSpan={7} className="ume-empty-cell">
                  No matching models
                </td>
              </tr>
            )}
...
function TableDeleteButton({
  ...
  return (
    <>
      <button
        ref={buttonRef}
        className="ume-table-delete-button"
        onClick={handleClick}
        type="button"
        title={isLink ? 'Delete link' : 'Delete'}
      >
        <Trash2 size={13} />
      </button>
      {open &&
...
function DetailPanel({
  ...
  return (
    <aside
      className={`ume-detail-panel open${resizing ? ' resizing' : ''}`}
      style={{ '--ume-detail-width': `${width}px` } as CSSProperties}
    >
      <div
        className="ume-detail-resizer"
      ...
      <header className="ume-detail-header">
        ...
        <div className="ume-detail-title">
          <strong>{titleForElement(element)}</strong>
          <code>
            {element.domain || 'unknown'}@{element.name || elementKey(element)}
          </code>
        </div>
        <span className="ume-kind-badge" style={{ background: color.bg, color: color.text }}>
          {color.label}
  ...
  return (
    <div className="ume-dialog-backdrop">
      <section className="ume-dialog ume-dialog-wide ume-create-node-dialog">
        ...
        <footer>
          <button className="ume-secondary-inline" onClick={onClose} type="button">
            Cancel
          </button>
...
function UploadDialog({
  ...
  const duplicateIds = useMemo(() => {
    ...
    return parseUploadPreview(text)
      .filter((element) => ids.has(elementKey(element)))
      .map(elementKey);
  }, [elements, text]);
      ...
      <section className="ume-dialog ume-dialog-wide">
        ...
        <div className="ume-upload-toolbar">
          ...
          {duplicateIds.length > 0 && (
            <span>
              {duplicateIds.length} existing id{duplicateIds.length === 1 ? '' : 's'} will be overwritten in draft.
            </span>
          )}
        </div>
...
function NodePickerTable({
  ...
  const kindOptions = useMemo(
    () => [...new Set(rows.map((row) => row.kind))].sort((left, right) => kindRank(left) - kindRank(right)),
    [rows]
  );
  ...
  return (
    <div className="ume-node-picker">
      ...
      <div className="ume-node-picker-table">
        <table>
          ...
          <tbody>
            ...
            {pagedRows.length === 0 && (
              <tr>
                <td colSpan={4} className="ume-empty-cell">
                  No matching nodes
                </td>
              </tr>
            )}
      ...
      <div className="ume-node-picker-footer">
        <span>
          {filteredRows.length} items · page {page}/{totalPages}
        </span>
        <button disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))} type="button">
          ‹
        </button>
...
function ConnectDialog({
  ...
  function confirm() {
    if (!selectedSource || targetCount === 0) {
      return;
    }
      ...
      <section className="ume-dialog ume-dialog-wide ume-connect-dialog">
        <header>
          <div>
            ...
            <span>
              {source
                ? `Create links from ${titleForElement(source)}`
                : 'Select a source node and target node to create a link.'}
            </span>
          </div>
          <button className="ume-icon-button subtle" onClick={onClose} type="button">
            <X size={15} />
        ...
        {!source && (
          <div className="ume-connect-steps">
            <button className={step === 'source' ? 'active' : ''} onClick={() => setStep('source')} type="button">
              1. Source {selectedSource ? `(${titleForElement(selectedSource)})` : ''}
            </button>
            <button
              ...
              type="button"
            >
              2. Target {targetCount > 0 ? `(${targetCount})` : ''}
            </button>
          </div>
        )}
        <div className="ume-dialog-body">
        ...
        <footer>
          ...
          <div>
            ...
            <button
              className="ume-primary-button"
              disabled={!selectedSource || targetCount === 0}
              onClick={confirm}
              type="button"
            >
...
function DiffDialog({
  ...
  const serverById = useMemo(
    () => new Map(serverElements.map((element) => [elementKey(element), element])),
    [serverElements]
  );
  const changes = [
    ...diff.added.map((element) => ({ type: 'added' as const, element, original: null })),
    ...diff.modified.map((element) => ({
      type: 'modified' as const,
      element,
      original: serverById.get(elementKey(element)) || null,
    })),
    ...
    <div className="ume-drawer-backdrop">
      <section className="ume-diff-drawer">
        ...
        <div className="ume-diff-summary">
          <span>
            Added <strong>{diff.added.length}</strong>
          </span>
          <span>
            Modified <strong>{diff.modified.length}</strong>
          </span>
          <span>
            Deleted <strong>{diff.deleted.length}</strong>
          </span>
        </div>
        <div className="ume-dialog-body">
          ...
          <div className="ume-diff-viewer">
            {selected ? (
              <>
                ...
                <div className="ume-diff-sxs">
                  <div className="ume-diff-pane">
                    <CodeEditor
                      value={originalJson}
                      language="json"
                      height="100%"
...
function normalizeMModelPayload(payload: unknown): MModelElement[] {
  if (Array.isArray(payload)) {
    return payload.map(normalizeMModelElement);
  }
  if (isObject(payload) && Array.isArray(payload.elements)) {
    return payload.elements.map(normalizeMModelElement);
  }
  if (isObject(payload) && Array.isArray(payload.items)) {
    return payload.items.map(normalizeMModelElement);
  }
  if (isObject(payload) && Array.isArray(payload.rows)) {
    return payload.rows.map((row) => normalizeMModelElement(row));
  }
  if (isObject(payload)) {
    return [normalizeMModelElement(payload)];
  }
...
function normalizeMModelElement(value: unknown): MModelElement {
  if (!isObject(value)) {
    throw new Error('Each MModel element must be an object.');
  }
  const element = rowToElement(value);
  if (!element.kind) {
    throw new Error(`Element ${elementKey(element) || '<unknown>'} is missing kind.`);
  }
  if (!element.domain) {
    throw new Error(`Element ${element.name || '<unknown>'} is missing domain.`);
  }
  if (!element.name) {
    throw new Error(`Element ${element.domain || '<unknown>'} is missing name.`);
  }
  return element;
}
