    ...
    <div className="ume-sidebar-body">
      ...
      {diffTotal > 0 && (
        <button
          ...
          type="button"
          title={
            focusableDiffCount > 0 ? 'Focus added and modified draft elements' : 'Deleted elements cannot be focused'
          }
        >
      ...
      <div className="ume-filter-list">
        {stats.domainEntries.slice(0, 12).map(([domain, count]) => {
          ...
          return (
            <button
              key={domain}
              className={`ume-filter-row ${active ? 'active neutral' : ''}`}
      ...
      {open &&
        ...
        ReactDOM.createPortal(
          <div className="ume-diff-help-panel" style={{ left, top, width: panelWidth } as CSSProperties}>
            ...
            <p>
              Changes are applied only after <b>Confirm &amp; Submit</b>.
            </p>
          </div>,
          document.body
        )}
  ...
  return (
    <div className="ume-filter-bar">
      <InfoTooltip>
        ...
        <p>
          <b>Focus</b> shows selected items and nearby related items.
        </p>
        <p>
          <b>Stack filters</b> narrows that focused view with search, type, or domain filters.
        </p>
      ...
      {hasFocus && (
        <>
          ...
          <button
            className={`ume-filter-stack ${filterStacking ? 'active' : ''}`}
      ...
      {kindFilters.map((kind) => {
        ...
        return (
          <ActiveFilterChip
            ...
            suffix={
              currentView === 'graph' && !kindActsAsGraphNode(kind, entitySetLinkDisplay) ? 'table only' : undefined
            }
