...
export function buildGraph(
  ...
  const nodeElements = elements.filter(
    (element) =>
      !isLinkElement(element) || (entitySetLinkDisplay === 'relative_link' && isEntitySetLinkElement(element))
  );
  ...
  for (const element of linkElements) {
    ...
    if (!source || !target || !nodeIds.has(source) || !nodeIds.has(target) || !sourceElement || !targetElement) {
      continue;
    }
  ...
  const nodes = fallbackLayoutNodes(nodeElements).map(({ element, position }) => {
    ...
    const width = entityLinkNode
      ? Math.min(190, Math.max(68, entityLinkTypeForEdge(element).length * 8 + 38))
      : nodeWidth;
  ...
  const edges: Array<Edge<ExplorerEdgeData>> = validEdges.map(
    ({ id, element, source, target, sourceElement, targetElement, kind }) => ({
      id,
      type: 'mmodel',
      source,
      target,
...
export async function layoutGraphWithGraphviz(model: GraphModel): Promise<GraphModel> {
  ...
  for (const item of parsed.objects || []) {
    ...
    if (Number.isFinite(xRaw) && Number.isFinite(yRaw)) {
      positions.set(item.name, { x: xRaw, y: yRaw });
    }
  }
  if (positions.size === 0) {
    return model;
