...
export function filterFlatElements(
  ...
  return elements.filter((element) => {
    ...
    if (focusSet && !focusSet.has(key)) {
      return false;
    }
    if (focusSet && !filterStacking) {
      return true;
    }
    if (kinds.length > 0 && !kinds.includes(element.kind)) {
      return false;
    }
    if (domains.length > 0 && !domains.includes(element.domain || 'unknown')) {
      return false;
    }
...
export function filterGraphElements(
  ...
  for (const element of graphNodeElements) {
    ...
    if (!elementMatchesFlatFilters(element, fullTextFilters, graphKindFilters, domains)) {
      continue;
    }
    selectedNodeIds.add(key);
  }
  ...
  for (const element of graphNodeElements) {
    const key = elementKey(element);
    if (!isEntitySetLinkElement(element) || !selectedNodeIds.has(key)) {
      continue;
    }
...
export function buildSearchIndex(elements: MModelElement[]): SearchIndex {
  ...
  const entries = elements.map((element) => {
    ...
    for (const token of recommendationTokens) {
      if (!isRecommendationToken(token, allKinds, allDomains)) {
        continue;
      }
  ...
  const topTokens = [...tokenCounts.entries()]
    .sort(
      (left, right) =>
        tokenQuality(right[0]) - tokenQuality(left[0]) || right[1] - left[1] || left[0].localeCompare(right[0])
    )
    .slice(0, 200)
...
function isRecommendationToken(token: string, allKinds: Set<string>, allDomains: Set<string>) {
  ...
  if (recommendationStopWords.has(token)) {
    return false;
  }
  if (allKinds.has(token) || allDomains.has(token)) {
    return false;
  }
  return true;
}
...
export function searchIndexSearch(index: SearchIndex, query: string, limit = 20): SearchEntry[] {
  ...
  if (needles.length === 0) {
    return index.entries.slice(0, 8);
  }
    ...
    .map((entry) => {
      ...
      for (const needle of needles) {
        ...
        if (lowerTitle.includes(needle)) {
          ...
          termScore = 10;
        } else if (entry.element.kind.toLowerCase().includes(needle)) {
          termScore = 7;
        } else if ((entry.element.domain || '').toLowerCase().includes(needle)) {
          termScore = 7;
        } else if (entry.tokens.some((token) => token.toLowerCase() === needle)) {
          termScore = 5;
        } else if (entry.tokens.some((token) => token.toLowerCase().startsWith(needle))) {
          termScore = 3;
        } else if (entry.tokens.some((token) => token.toLowerCase().includes(needle))) {
          termScore = 1;
        }
        if (termScore === 0) {
          allNeedlesHit = false;
...
export function countEntries(entries: SearchEntry[], getKey: (entry: SearchEntry) => string) {
  ...
  for (const entry of entries) {
    counts.set(getKey(entry), (counts.get(getKey(entry)) || 0) + 1);
  }
  return counts;
}
...
export function aliasForElements(elements: MModelElement[]) {
  ...
  for (const element of elements) {
    ...
    alias.set(key, key);
    if (element.name) {
      alias.set(element.name, key);
    }
    if (element.domain && element.name) {
      alias.set(`${element.domain}.${element.name}`, key);
    }
  }
  return alias;
}
...
function rewriteEndpointForCopy(value: unknown, original: MModelElement, copy: MModelElement): unknown {
  if (!endpointMatchesElement(value, original)) {
    return value;
  }
  if (typeof value === 'string') {
    return copy.domain && copy.name ? `${copy.domain}.${copy.name}` : elementKey(copy);
  }
  if (isObject(value)) {
    return {
      ...value,
...
export function columnForKind(kind: string) {
  ...
  if (kind === 'entity_set_link') {
    return 1;
  }
  if (kind === 'metric_set') {
    return 2;
  }
  if (kind === 'log_set' || kind === 'trace_set' || kind === 'event_set' || kind === 'profile_set') {
    return 3;
  }
  if (kind === 'runbook_set' || kind === 'explorer') {
    return 4;
  }
...
export function kindRank(kind: string) {
  ...
  if (linkIndex >= 0) {
    return nodeKindOrder.length + linkIndex + 1;
  }
...
export function colorForKind(kind: string): KindColor {
  return (
    kindColors[kind] || {
      ...
      abbrev: formatKindLabel(kind).slice(0, 2).toUpperCase(),
    }
  );
}
...
export function descriptionForElement(element: MModelElement): string {
  ...
  if (isObject(description)) {
    return optionalString(description.zh_cn) || optionalString(description.en_us) || '';
  }
  return '';
}
...
export function tableSortValue(element: MModelElement, key: 'name' | 'domain' | 'kind' | 'description'): string {
  if (key === 'name') {
    return titleForElement(element).toLowerCase();
  }
  if (key === 'domain') {
    return (element.domain || '').toLowerCase();
  }
  if (key === 'kind') {
    return labelForKind(element.kind).toLowerCase();
  }
...
export function tagsForElement(element: MModelElement): string[] {
  ...
  return source
    .map((item) => {
      ...
      if (isObject(item)) {
        return optionalString(item.name) || optionalString(item.type) || '';
      }
      return '';
    })
    .filter(Boolean)
